...
Our primary interest in these data will be in exploring patterns in broad taxanomic groups such as *Hard coral* and *algae*. Since the machine learning tools behind ReefCloud are tuned to classify to finer taxonomic resolutions, next task will be to use a lookup table so as to assign the higher order group codes (HC and A) to the data.
...
labelset_nt |> datawizard::data_codebook() |> knitr::kable() 
```
:::
...
```{r}
GROUPS <- dat_nt |> pull(GROUP) |> unique()
data.filler <- dat_nt |>
  dplyr::select(
    starts_with("site"),
    survey_start_date,
